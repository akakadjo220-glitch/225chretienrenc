...
export const Forum: React.FC = () => {
    ...
    return (
        <div className="max-w-3xl mx-auto px-4 pb-12 font-sans bg-slate-50 min-h-screen">
            ...
            {currentUser && (
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-3 sm:p-4 mb-6">
                    <div className="flex items-center gap-3 pb-3 border-b border-slate-100">
                        ...
                        <button 
                            ...
                            className="flex-1 text-left bg-slate-100/90 hover:bg-slate-200/90 text-slate-600 rounded-full px-4 py-2.5 text-xs sm:text-sm font-medium transition truncate cursor-pointer min-w-0"
                        >
                            <span className="truncate block">
                                Qu'avez-vous à l'esprit, {(currentUser.full_name || currentUser.name || 'frère / sœur').split(' ')[0]} ? 🙏
                            </span>
                        </button>
                    </div>
                    {/* Actions en grille 3 colonnes égales & aérées */}
