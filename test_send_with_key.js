...
async function testSend() {
  const sessionsToTest = [session1, session2, '225chretien', 'babipass'];

  for (const s of sessionsToTest) {
    try {
      console.log(`🚀 Testing send-text with session "${s}"...`);
      ...
      if (res.ok) {
        console.log(`🎉 SUCCESS SENDING MESSAGE WITH SESSION "${s}"!`);
      }
    } catch (e) {
      console.error(e);
    }
